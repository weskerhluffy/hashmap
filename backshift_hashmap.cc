...
int BackshiftHashMap::Get(const std::string& key, std::string* value) {
	...
	for (i = 0; i < probing_max_; i++) {
		...
		if (buckets_[index_current].entry == NULL || i > probe_distance) {
			break;
		}

		if (key.size() == buckets_[index_current].entry->size_key
				&& memcmp(buckets_[index_current].entry->data, key.c_str(),
						key.size()) == 0) {
			*value = std::string(
					buckets_[index_current].entry->data + key.size(),
...
int BackshiftHashMap::FillInitIndex(uint64_t index_stored,
		uint64_t *index_init) {
	if (buckets_[index_stored].entry == NULL)
		return -1;
	*index_init = buckets_[index_stored].hash % num_buckets_;
	return 0;
}

int BackshiftHashMap::FillDistanceToInitIndex(uint64_t index_stored,
		uint64_t *distance) {
	if (buckets_[index_stored].entry == NULL)
		return -1;
...
void BackshiftHashMap::GetMetadata(
		std::map<std::string, std::string>& metadata) {
	metadata["name"] = "backshift";
	char buffer[1024];
	sprintf(buffer,
			"{\"num_buckets\": %" PRIu64 ", \"probing_max\": %" PRIu64 "}",
			num_buckets_, probing_max_);
